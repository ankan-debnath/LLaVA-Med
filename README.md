...
```Shell
python -m llava.serve.model_worker --host 0.0.0.0 --controller http://localhost:10000 --port 40000 --worker http://localhost:40000 --llava_med-path microsoft/llava-med-v1.5-mistral-7b --multi-modal
```
...
```Shell
python -m llava.serve.model_worker --host 0.0.0.0 --controller http://localhost:10000 --port 40000 --worker http://localhost:40000 --llava_med-path microsoft/llava-med-v1.5-mistral-7b --multi-modal --num-gpus 2
```
Wait until the process finishes loading the model and you see "Uvicorn running on ...".
...
#### Send a test message
```Shell
python -m llava.serve.test_message --llava_med-name llava-med-v1.5-mistral-7b --controller http://localhost:10000
```
...
PYTHONPATH=. python llava/eval/model_vqa.py \
    ...
    --llava_med-path microsoft/llava-med-v1.5-mistral-7b \
